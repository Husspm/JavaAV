...
//	gl_FragDepth = map(col, 0, 1, -10, 10);
	if(col < 0.3){
		...
		gl_FragColor = mix(vec4(col, col / 4, col /8, trans),texture2D(texture2, vertTexCoord.xy * 0.3) ,0.5);
	}else if(col <  0.6){
		trans = map(transMod.y, 0, 3, 0, 0.5);
		col = abs(map(col, 0.3, 0.599, -1, 1));
		gl_FragColor = mix(vec4(col, col / 2, col /3, trans),texture2D(texture, vertTexCoord.xy * 0.3) ,0.5);
	}else{
	trans = map(transMod.y, 0, 4, 0, 0.5);
		col = map(col, 0.6, 1, 0.4, 0.96);
		gl_FragColor = vec4(col * 2, col * 3, col * 4, trans);
	}
}
void main() {
	vec2 pos = gl_FragCoord.xy * (offset * transMod.xy);
	...
	if (transMod.y < (0.7)){
		...
		vec3 altColor = texture(texture, vec2(offsetCoord.x + sin(ang), offsetCoord.y + cos(ang))).rgb - texture(texture2, pos).rgb * sin(offset.y);
		...
		if(texColor.b < 0.201){
			...
			gl_FragColor = clr[int(round(vertTexCoord.x))] * sin(transMod.z);
		}else{
