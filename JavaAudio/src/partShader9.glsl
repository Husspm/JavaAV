...
//	vec3 texColor2 = texture2D(texture, vertTexCoord.xy + (offset2.xy / 10000)).rgb * cMod * 10;
	...
	vec2 wavelines = vec2(vertTexCoord.x + (sin(time * midVol)) - (sin(ang * (vertTexCoord.x - 0.5) + sin(time * (cMod * 100))) / (cMod * max(2, midVol / 3.5))), vertTexCoord.y * cos(ang) / distance(vertTexCoord.xy, offset2.xy) * (lightDist * 15));
