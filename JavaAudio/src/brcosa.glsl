...
void main() {
	...
	vec4 finalMix = vec4(brightness * conColor, 0.4820);
	float blackCheck = findMax(finalMix);
	if (blackCheck < 0.412){
		vec3 newTexColor = texture2D(texture, vec2(vertTexCoord.x * 0.5, vertTexCoord.y * 0.5)).rrr;
		gl_FragColor = vec4(newTexColor * 5, 0.5);
	}else if(isWhite(finalMix)){
		float col = abs(map(finalMix.r, 0.9, 1.9, -1, 1));
		gl_FragColor = vec4(col, col, col, finalMix.a / 3);
	}else{
