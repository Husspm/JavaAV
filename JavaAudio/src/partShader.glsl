...
//	gl_FragDepth = max(distance(vertTexCoord.xy, offset.xy), distance(vertTexCoord.xy, offset2.xy));
	...
	float angMult = map(abs(noise1(offset)) * 10000, 0, 10000, 2, 5);
...
//	vec3 texColor2 = texture2D(texture, vertTexCoord.xy + (offset2.xy / 10000)).rgb * cMod * 10;
	...
	vec2 wavelines = vec2(vertTexCoord.x - (sin(ang * (vertTexCoord.x * offset.z) / (cMod * max(2, midVol / 3.5)))), vertTexCoord.y * cos(ang) / distance(vertTexCoord.xy, offset2.xy) * sin(time / 1000 / vertTexCoord));
	vec2 wavelines2 = vec2(vertTexCoord.x - (cos(ang * (vertTexCoord.x * offset.z) + tan(time * (cMod * 0.1))) / (cMod * max(2, midVol / 1.5))), vertTexCoord.y * tan(ang) / distance(vertTexCoord.xy, offset2.xy) / midVol * 2);
	vec3 mixColors = texture2D(texture, wavelines2 / lightDist).rgb;
	mixColors.rb *= lightDist;
	mixColors.g *= lightDist2;
	...
	vec3 finalMix = mix(mixColors3, vec3(mixColors3.r * lightDist, mixColors3.g * lightDist2, mixColors3.b * lightDist), 0.6);
